...
An incremental model is best because the initial requirements are simple and well understood, while future features such as online appointments and automated reminders can be added over time. The client is flexible with delivery time and expects ongoing enhancements, which aligns well with incremental development.



## Project #3: Nuclear Power Plant  
**SDLC Model:** **Waterfall (Plan-Driven)**

A plan-driven waterfall model is best because of the safety-critical nature of the system and the severe consequences of failure. The need for extensive verification, robustness, long system lifespan, and regulatory compliance requires detailed upfront specification, documentation, and validation before deployment.
...
Agile development is best because requirements are evolving and the goal is rapid delivery of playable versions for testing by internal and external users. Frequent feedback from teachers and administrators allows the team to refine gameplay, educational value, and usability through short development iterations.
